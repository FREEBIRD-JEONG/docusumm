...
describe("YouTube remote success integration", () => {
  const userId = "user-youtube-success";
  let credits = 3;
  ...
  beforeEach(() => {
    ...
    mockedCompleteSummaryJob.mockImplementation(async (summaryId, jobId, summaryText) => {
      const summary = summaries.get(summaryId);
    ...
    mockedSummarizeWithFallback.mockResolvedValue({
      summaryText: ["TL;DR", "- 핵심1", "- 핵심2", "- 핵심3", "", "전체 요약", "본문"].join("\n"),
    });
  });

  it("completes youtube summary without refund when remote transcript path succeeds", async () => {
    ...
    expect(workerData.completed).toBe(1);
    expect(workerData.failed).toBe(0);
    ...
    expect(summaryData.status).toBe("completed");
    expect(summaryData.summary).toContain("TL;DR");
    expect(summaryData.record.errorMessage).toBeNull();
    expect(credits).toBe(2);
    expect(mockedRestoreUserCredit).not.toHaveBeenCalled();
    expect(mockedSendSummaryCompletedEmail).toHaveBeenCalledTimes(1);
  });
});
